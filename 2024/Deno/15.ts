...
const moveRobot = (
  ...
  while (true) {
    ...
    if (
      dirItems[dirItems.length - 1] === "#" ||
      grid[rY + i * dy][rX + i * dx] === "."
    ) {
      break;
    }
    i++;
  ...
  for (let j = 0; j < dirItems.length; j++) {
    grid[rY + j * dy][rX + j * dx] =
      dirItems[(j - 1 + dirItems.length) % dirItems.length];
  }
...
console.log(
  gridArr.reduce(
    (sum, line, y) =>
      sum +
      line.reduce(
        (rowSum, item, x) => rowSum + (item === "O" ? 100 * y + x : 0),
        0,
      ),
    0,
  ),
...
console.log(
  newGrid.reduce(
    (sum, line, y) =>
      sum +
      line.reduce(
        (rowSum, item, x) => rowSum + (item === "[" ? 100 * y + x : 0),
        0,
      ),
    0,
  ),
